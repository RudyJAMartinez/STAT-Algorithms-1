...
setwd("/Users/rudymartinez/Desktop/MSDA/Fall 2020/STA 6443_Algorithms I/STAT-Algorithms-1/Week 6")  # need to change this path
...
# H0: mu(0.5) = mu(1) = mu(2) OR No Dose Effect | Ha:Dose Effect (At least one dose group has a different mean)
aov.tooth= aov(Toothlength~Dose, data=tooth)   # aov()

LeveneTest(aov.tooth)   # Passing in the AOV results - If levene test is significant (null: homoscedasticity), use  Welch correction
...
#Both of these do the same thing (2 and 3)
...
par(mfrow=c(1,1))   # normality check 
plot(aov.tooth2, 2)
